...
### ⚠️ 절대 잊지 말 것 — Discord 완료 알림

**Discord를 통해 전달된 메시지에 응답을 마친 후, 반드시 아래를 실행할 것:**

```bash
touch /tmp/aeong_fin
```

이 플래그가 없으면 버블리님 Discord에 완료 알림이 **절대 전송되지 않음**.
- `claude -p -c` 방식이 제거된 이후 Claude가 직접 생성해야 함
- 작업 중단/거절/완료 **모든 경우**에 실행
- 응답의 맨 마지막 Bash 호출로 반드시 포함할 것
